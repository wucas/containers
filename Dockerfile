...
ADD ./test.sh .
RUN ./test.sh -v ${GAP_VERSION} -d 0 -t minimal \
    && rm test.sh
...
ADD ./test.sh .
RUN ./test.sh -v ${GAP_VERSION} -d 0 -t full \
    && rm test.sh
...
ADD ./test.sh .
RUN ./test.sh -v ${GAP_VERSION} -d 1 -t minimal \
    && rm test.sh
...
ADD ./test.sh .
RUN ./test.sh -v ${GAP_VERSION} -d 1 -t full \
    && rm test.sh
